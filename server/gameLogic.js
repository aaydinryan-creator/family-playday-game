...
const POT_BATTLE_THRESHOLD = 3000;
...
function ensurePotBattleState(room) {
  if (!room) return;
  ...
  if (!Number.isFinite(Number(room.potBattleThreshold))) {
    room.potBattleThreshold = POT_BATTLE_THRESHOLD;
  }

  if (!Number.isFinite(Number(room.lastPotBattleAt))) {
    room.lastPotBattleAt = 0;
  }
}

function markPotBattleReady(room) {
  if (!room) return false;

  ensurePotBattleState(room);

  if (room.potBattleReady) {
    return false;
  }

  if (room.pot >= room.potBattleThreshold) {
    room.potBattleReady = true;
    room.potBattleTriggered = true;
    room.lastPotBattleAt = room.pot;
    return true;
  }
...
const DEAL_CARDS = [
  ...
  { text: "You sold a dusty lamp like it was luxury decor. Collect $530.", amount: 530 },
...
function handleTile(room, player, roll) {
  ...
  let cards = [];
  let soundCue = null;
  let potBattleTriggeredThisTurn = false;
  ...
  switch (tile.type) {
    ...
    case "ski": {
      ...
      potBattleTriggeredThisTurn = addToPot(room, tile.cost);
    ...
    case "walk": {
      ...
      room.players.forEach((otherPlayer) => {
        if (otherPlayer.id !== player.id) {
          ...
          room.bank += amount;
          potBattleTriggeredThisTurn = addToPot(room, amount) || potBattleTriggeredThisTurn;
          total += amount;
        }
      });
  ...
  if (potBattleTriggeredThisTurn) {
    message += ` POT BATTLE is now ready at ${formatMoney(room.pot)}!`;
  }
  ...
  return {
    ...
    pot: room.pot,
    potBattleReady: room.potBattleReady,
    potBattleTriggered: potBattleTriggeredThisTurn,
    potBattleThreshold: room.potBattleThreshold
  };
}
